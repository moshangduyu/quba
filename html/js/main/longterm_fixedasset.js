      ...
      */
      getList:function(currentPage,data){
        var _this = this;
				...
				$.ajax({
					type:"get",
					url:urlPath + '/web/fixedasset/createDesc/long/fixedassets',
          data:data,
					dataType: 'json',
            ...
            if(res.data.rows.length==0){
              ...
              $.each(res.data.rows,function(index,item){
                ...
                if(item.description==undefined){
  								html+='<div class="list-txt"><p>未填写</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a target="_blank" href="longterm_fixedassetDetaile.html?fixedasset='+ item.id +'">查看详情</a> </div></div></div></li>';
  							}else{
  								var description = _this.cutstr(item.description,165);
  								html+='<div class="list-txt"><p>'+ description +'</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a target="_blank" href="longterm_fixedassetDetaile.html?fixedasset='+ item.id +'">查看详情</a> </div></div></div></li>';
                }
              })
	            // 切换正在加载状态及搜索栏右侧页数
							if(res.data.rows.length==0){
