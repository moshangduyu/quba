  ...
  var login = require("../module/login.js"),
    ...
    var loneDetaile = {
        ...
        $.ajax({
          ...
          success:function(res){
            ...
            if(res.data.title.length<=12){
              DetaileCard+='<div class="detaile-topinfobox-head" style="margin-bottom: 63px;">';
            }else{
              DetaileCard+='<div class="detaile-topinfobox-head" style="margin-bottom: 32px;">';
            }
            ...
            if(code[0] == 100){
              ...
              DetaileCard+=' <li>技术类型：非专利</li>\
                             <li>技术所属地：'+ res.data.regionFullName +'</li>';
            }
            DetaileCard+='<li>成 熟 度：'+ res.data.maturityName +'</li>\
                          <li>交易方式：'+ res.data.tradingModeName +'</li>';
            if(res.data.startPrice>=1000){
              DetaileCard+='<li style="margin-bottom: 5px;">转让底价：<span class="detaile-startprice">￥'+ (res.data.startPrice/10000) +'万</span>';
            }else{
              DetaileCard+='<li style="margin-bottom: 5px;">转让底价：<span class="detaile-startprice">￥'+ (res.data.startPrice) +'元</span>';
            }
      ...
      createjingjia:function(){
        ...
        $.ajax({
          type:"get",
          url:urlPath + '/web/userCenter/tech/isQuoted?tech='+ _this.project,
          dataType: 'json',
              ...
              $('.btn-jingjia').unbind('click').on('click',function(){
                ...
                $('.btn-submitjingjia').on('click', function(event) {
                  ...
                  if(price!=''){
                    $.ajax({
                      type:"post",
                      url:urlPath + '/web/userCenter/tech/quote',
                      data:{tech:_this.project,price:price},
                      dataType: 'json',
      ...
      jingjia:function(){
        ...
        $.ajax({
          type:"get",
          url:urlPath + '/web/tech/biddings?tech='+this.project,
          dataType: 'json',
      ...
      scrollTab:function(){
        ...
        var arr = [];
        arr.push(decTop,picTop,processTop,dataTop,safeTop,userTop);
