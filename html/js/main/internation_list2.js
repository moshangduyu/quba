    ...
    var  jingjia = {
      ...
      getList:function(currentPage){
        ...
        var _this = this;
				$.ajax({
					type:"get",
					url:urlPath + '/web/int/createDesc/bidding/techs',
                ...
                if(item.currency=="CNY"){
                  ...
                  html+='<div class="list-txt"><span>转让底价</span><span>$'+ (item.startPrice/10000) +'万</span></div>';
								...
								if(item.description==undefined){
									html+='<div class="list-txt"><p>未填写</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a href="jinjia_detaile.html?tech='+ item.id +'">查看详情</a> </div></div></div></li>';
								}else{
									var description = _this.cutstr(item.description,165);
									html+='<div class="list-txt"><p>'+ description +'</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a href="jinjia_detaile.html?tech='+ item.id +'">查看详情</a> </div></div></div></li>';
								}
              })
            }
            $('.longterm-content .spinner').hide();
