  ...
  $(function(){
    var  longterm = {
      ...
      */
      navSwith:function(){
        var _this = this;
        var obj = ['nav_industrys','nav_tradingmodes','nav_maturitys'];
                  ...
                  title = $('#search_input').val();
							...
							if($(this).parent().parent().attr('id')=='typeTree'){
								if($(this).attr('data-key')=='100'){
									$('#patent-list').show();
									$('#nopatent-list').hide();
									$('#nopatent-list>ul>li').removeClass('active');
								}else{
									$('#patent-list').hide();
									$('#nopatent-list').show();
							...
							$('#patent-list>ul>li,#nopatent-list>ul>li').unbind('click').on('click',function(){
								if($(this).hasClass('active')){
									return false;
								}else{
									$(this).addClass('active').siblings('li').removeClass('active');
									if($(this).parent().parent().attr('id') == 'patent-list'){
										techType = $('#patent-list>ul>li.active').attr('data-key')!=undefined?$('#patent-list>ul>li.active').attr('data-key'):"";
									}else{
										techType = $('#nopatent-list>ul>li.active').attr('data-key')!=undefined?$('#patent-list>ul>li.active').attr('data-key'):"";
									}
									$('.longterm-listbox').html('');
		              _this.getList("",industry,techType,tradingMode,maturity,title);
								}
          ...
          success:function(res){
            ...
            _this.navSwith();
	            ...
	            $('.longterm-nav-row').show();
							$('#patent-list, #nopatent-list').hide();
						}
          }
        })
      },
            ...
            $.each(res.data,function(index,item){
							...
							if(item.code == 100){
								$.each(item.childs,function(index, el) {
									parentList+='<li data-key="'+ el.code +'"><span>'+ el.name +'<i></i></span></li>';
								});
							}else{
								$.each(item.childs,function(index, el) {
									noParentList+='<li data-key="'+ el.code +'"><span>'+ el.name +'<i></i></span></li>';
								});
							}
            })
            $('#typeTree>ul').append(html);
						$('#patent-list>ul').append(parentList);
						$('#nopatent-list>ul').append(noParentList);
						_this.navSwith();
          }
        })
			},
      ...
      */
      getList:function(currentPage,industry,techType,tradingMode,maturity,title){
        var _this = this;
				$.ajax({
					type:"get",
					url:urlPath + '/web/tech/createDesc/long/techs',
                      ...
                      <ul class="list-tag">\
                          ...
                          <li>'+ item.maturityName +'</li>\
								...
								if(item.description==undefined){
									html+='<div class="list-txt"><p>未填写</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a target="_blank" href="longterm_detaile.html?tech='+ item.id +'">查看详情</a> </div></div></div></li>';
								}else{
									var description = _this.cutstr(item.description,165);
									var length = item.title.length;
									if(length>12){
										html+='<div class="list-txt"><p>'+ description +'</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3 style="font-size:18px;">'+ item.title +'</h3> <a target="_blank" href="longterm_detaile.html?tech='+ item.id +'">查看详情</a> </div></div></div></li>';
									}else{
										html+='<div class="list-txt"><p>'+ description +'</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a target="_blank" href="longterm_detaile.html?tech='+ item.id +'">查看详情</a> </div></div></div></li>';
									}
								}
              })
            }
