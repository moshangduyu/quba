    ...
    var  longterm = {
      ...
      getList:function(currentPage){
        var _this = this;
				$.ajax({
					type:"get",
					url:urlPath + '/web/int/createDesc/bidding/fixedassets',
	                ...
	                if(bidRangeNum>=10000){
	                  ...
	                  var bidRang = bidRangeNum.toFixed(0).replace(/(\d)(?=(\d{3})+$)/g, '$1,')+'元';
								...
								html+='<div class="list-txt"><span>转让底价：</span><span>'+ startPrice +'</span></div>\
											 <div class="list-txt"><span>转让底价：</span><span>'+ bidRang +'</span></div>';

                if(item.description==undefined){
  								html+='<div class="list-txt"><p>未填写</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a href="jinjia_fixedassetDetaile.html?fixedasset='+ item.id +'">查看详情</a> </div></div></div></li>';
  							}else{
  								var description = _this.cutstr(item.description,165);
  								html+='<div class="list-txt"><p>'+ description +'</p></div></div></a><div class="hoverlist"> <div class="inner"> <h3>'+ item.title +'</h3> <a href="jinjia_fixedassetDetaile.html?fixedasset='+ item.id +'">查看详情</a> </div></div></div></li>';
                }
              })
	            $('.longterm-content .spinner').hide();
