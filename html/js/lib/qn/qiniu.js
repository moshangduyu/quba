...
function QiniuJsSDK() {
    ...
    this.uploader = function(op) {
        		...
        		ajax.onreadystatechange = function() {
        			if (ajax.readyState === 4 && ajax.status === 200) {
        				...
        				if(res.status==0){
        					...
        					that.keyIndex=that.keys.length;
        					that.currentKeyIndex=0;
        				}
        				else
        					alert(res.message);
        			}
        ...
        var getUpKeys = function() {
            var ajax = that.createAjax();
            ajax.open('GET', that.keys_url, true);
            ajax.setRequestHeader("If-Modified-Since", "0");
            ajax.onreadystatechange = function() {
                if (ajax.readyState === 4 && ajax.status === 200) {
                    var res = that.parseJSON(ajax.responseText);
        ...
        var getFileKey = function(up, file, func) {
            var key = '',
                unique_names = false;
            key=that.keys[that.currentKeyIndex];
            that.currentKeyIndex=that.currentKeyIndex+1;
            if(that.currentKeyIndex==that.keyIndex)
            	getUpKeys();
            /*if (!op.save_key) {
